...
<title>${escape(projected.title)}</title>
...
</head><body><main><h1>${escape(projected.title)}</h1>
<p class="meta">DictAI · ${escape(Instant.ofEpochMilli(projected.updatedAt).toString())}</p>
""")
        val parts = NoteImageMarkers.parts(projected)
