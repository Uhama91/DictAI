...
for library in "${required_libs[@]}"; do
    ...
    "$readelf" -h "$path" | rg 'Class:.*ELF64' >/dev/null
    "$readelf" -h "$path" | rg 'Machine:.*AArch64' >/dev/null
    "$readelf" -lW "$path" | awk '
        $1 == "LOAD" {
            found = 1
            if ($NF != "0x4000") bad = 1
    ...
    '
    if "$readelf" -SW "$path" | rg '\.debug_' >/dev/null; then
...
require_needed() {
    local library=$1
    local dependency=$2
    "$readelf" -d "$bundle_dir/$library" | rg "Shared library: \[$dependency\]" >/dev/null
}

require_symbol() {
    local library=$1
    local symbol=$2
    "$readelf" -Ws "$bundle_dir/$library" | rg "GLOBAL.*$symbol$" >/dev/null
}
